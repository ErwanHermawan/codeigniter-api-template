...
class Codeigniter_websocket
{
	 ...
	 * @var array
	 */
	protected $callback_type = array('auth', 'event', 'close', 'citimer', 'roomjoin', 'roomleave', 'roomchat', 'transaction');
...
class Server implements MessageComponentInterface
{
	...
	public function onMessage(ConnectionInterface $client, $message)
	{
		...
		if (valid_json($message)) {
			...
			if (!empty($datas->type) && $datas->type == 'transaction') {
				...
				if ($pass) {
					if (!empty($message)) {
						...
						foreach ($this->clients as $user) {
							...
							if (!empty($datas->recipient_id)) {
								if ($user->subscriber_id == $datas->recipient_id) {
									$this->send_notification($user, $message, $client);
									break;
								}
							} else {
								// Broadcast to everybody
	...
	protected function send_notification($user = array(), $message = array(), $client = array())
	{
		$ci =& get_instance();
		$data = json_decode($message, true);
		$database = get_db($data['user_id']);
		$o_data = (object) [];
		// check database
		if ($database) {
			$multi_where = [
			...
			$result = $ci->dynamic_model->get_sales_page($multi_where);
			$list = [];
			...
			foreach ($result as $val) 
			{
				$row = [];
				$row['data'] = $data;
				$row['sale_id'] = $val['sale_id'];
				$row['sale_date'] = f_date($val['sale_date']);
				$row['sale_time'] = $val['sale_time'];
				$row['customer_name'] = $val['customer_name'] !== null ? $val['customer_name'] : $val['customer'];
				$row['table_name'] = $val['table_name'];
				$row['total'] = $val['total'];
				$row['status'] = $val['status'];
				$row['total_items'] = $ci->dynamic_model->get_row_where('vw_sales_detail', $multi_where, 0 , 0);
				$list[] = $row;
			}

			$data_convert = json_encode($list);
			$o_data = str_replace(['[',']'],'',$data_convert);
			// $o_data = json_decode($replace);
		...
		if (!empty($this->CI->codeigniter_websocket->callback['event'])) {
			...
			// At this moment we have to check if we have authent callback defined
			call_user_func_array($this->CI->codeigniter_websocket->callback['event'],
				array((valid_json($o_data) ? json_decode($o_data) : $o_data)));
		...
		if ($this->CI->codeigniter_websocket->debug) {
			output('info',
				'Client (' . $client->resourceId . ') send \'' . $o_data . '\' to (' . $user->resourceId . ')');
		}
	}
